...
export class CameraRig {
  ...
  updateCameraTransform(dt) {
    ...
    // Underground the camera must stay INSIDE the room — beyond its walls
    // there is only the naked sky dome under the world.
    if (room) {
      this.desiredPosition.x = clamp(this.desiredPosition.x, room.minX + 0.7, room.maxX - 0.7);
      this.desiredPosition.z = clamp(this.desiredPosition.z, room.minZ + 0.7, room.maxZ - 0.7);
      const ceiling = room.ceilingAt
        ? room.ceilingAt(this.desiredPosition.z)   // the station's barrel arch
        : room.ceilingY - 0.35;                    // the siding's flat roof
